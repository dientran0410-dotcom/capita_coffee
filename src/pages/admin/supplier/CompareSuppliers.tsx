  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => window.history.back()} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-600 shadow-sm hover:bg-gray-50">
          ?
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Supplier Comparison</h1>
          <p className="text-sm text-gray-500">Product ID: <span className="font-mono text-blue-600">{productId}</span></p>
        </div>
      </div>

      {loading && <div className="p-10 text-center text-gray-500">Loading comparison data...</div>}
        ...
        rankingList.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-1">
            {rankingList.map((item, index) => (
              <div key={item.supplierId} className={`flex flex-col md:flex-row items-center justify-between rounded-xl border p-5 shadow-sm transition-all hover:shadow-md ${index === 0 ? "border-yellow-400 bg-yellow-50/30" : "border-gray-200 bg-white"}`}>
                <div className="flex w-full md:w-1/3 items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-900">
                      <Link to={`/admin/suppliers/${item.supplierId}`} className="hover:text-blue-600 hover:underline">
                        {item.supplierName}
                      </Link>
                    </h3>
                ...
                <div className="mt-4 flex w-full md:mt-0 md:w-2/3 justify-around border-t border-gray-100 pt-4 md:border-none md:pt-0">
                  ...
                  <div className="text-center">
                    ...
                    <p className={`mt-1 text-2xl font-black ${item.recommendationScore >= 80 ? "text-green-600" : item.recommendationScore >= 50 ? "text-amber-500" : "text-red-500"}`}>
